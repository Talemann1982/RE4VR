...
public:
    ...
    // Public-Block (order 60), gezeichnet von RE4VRMenu ueber RE4VRReload.
    void draw_public_shotgun_ratio();

    // Der Geschwister-Teil, an den der Mag-Drop delegiert.
    void set_adv(RE4VRReloadAdv* adv) { m_adv = adv; }

    // [HOLSTER-KETTE] In Lua wickeln reload2..reload5 ihre eigenen Fassungen um
    // __re4_reload_set_mag_in_hand -- der zuletzt geladene Wrapper wird zuerst
    // gefragt und reicht nur durch, wenn er die Waffe nicht kennt. Nativ haengt
    // sich der Traeger hier ein: liefert die Kette einen Wert, ist das die
    // Antwort, sonst laeuft der Original-Weg unten.
    void set_mag_chain(std::function<std::optional<bool>(bool)> f) {
        m_mag_chain = std::move(f);
    }

    // Zugang fuer die Hook-Lambdas und die Lua-Exporte (in Lua sind das
    // Upvalues der Datei-Closures). Gesetzt in on_initialize.
    static RE4VRReloadMain* instance() { return s_instance; }
...
private:
    static RE4VRReloadMain* s_instance;

    RE4VRReloadAdv* m_adv{nullptr};
    std::function<std::optional<bool>(bool)> m_mag_chain{};
    ...
    // [MERCS-TMP 2026-09-09] 4200 -> 42001, solange Krauser in Mercenaries
    // gefuehrt wird. Nur fuer die Keyframe-Bahn, s. Definition in der .cpp.
    int32_t kf_wid(int32_t wid) const;
    ...
    // [NOTFALL-SPUR 16.09.2026] Haelt einen Fehlerfall im Auswurf fest
    // (nur als Lua-Global __re4_reload_notfall -- der Fork schreibt keine Logs).
    void reload_notfall(const char* was);
    bool set_gun_loaded(int32_t n);

    // [TYPEWRITER-RUECKGABE 07.09.2026, Ansage des Users] Beim Oeffnen der
    // Schreibmaschine den gemerkten Magazinrest echt in die Waffe zurueck-
    // schreiben, damit das Spiel ihn MITSPEICHERT. Der Merker selbst ist reine
    // Mod-Buchhaltung und steht in keinem Savegame.
    void typewriter_restore_mag();
    bool m_typewriter_open{false};
    ...
    void ss_flash_pass();
    // [NACHZIEHEN] Insert-Position im LETZTEN Pass des Frames wiederholen.
    void insert_repos_late();
